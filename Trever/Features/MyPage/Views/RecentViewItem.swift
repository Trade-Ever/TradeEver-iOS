...
private extension RecentViewItem {
    ...
    private func resolvedEndDate() -> Date? {
        // Priority: live.endAt
        if let e = liveAuction?.endAt {
            print("RecentViewItem: endAt 문자열 = \(e)")
            if let d = parseISO8601(e) {
                print("RecentViewItem: 파싱된 endDate = \(d)")
                return d
            } else {
                print("RecentViewItem: endAt 파싱 실패")
            }
        } else {
            print("RecentViewItem: liveAuction?.endAt이 nil")
        }
        return nil
    }
